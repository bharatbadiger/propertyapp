namespace RealEstate.Controllers
{
    namespace RealEstate.Controllers
    {
        ...
        public class UsersController : ControllerBase
        {
            ...
            [HttpGet("{userId}/with-favorite-properties")]
            [ProducesResponseType(StatusCodes.Status200OK)]
            [ProducesResponseType(StatusCodes.Status404NotFound)]
            public async Task<ActionResult<ApiResponse<UserWithFavoriteProperties>>> GetUserWithFavoriteProperties([FromServices] UserRepository _userRepository, int userId)
            {
                var user = await _userRepository.GetUserWithFavoritePropertiesAsync(userId);

                if (user == null)
                {
                    var response = new ApiResponse<UserWithFavoriteProperties>(false, "User not found", null);
                    return NotFound(response);
                }

                return Ok(new ApiResponse<UserWithFavoriteProperties>(true, "User retrieved successfully", user));
            }

            [HttpGet("{userId}/favorite-properties")]
            [ProducesResponseType(StatusCodes.Status200OK)]
            [ProducesResponseType(StatusCodes.Status404NotFound)]
            public ActionResult<List<Property>> GetUserFavoriteProperties([FromServices] UserRepository _userRepository, int userId)
            {
                var favoriteProperties = _userRepository.GetUserFavoriteProperties(userId);
            ...
            // POST /api/users/{userId}/favorite-properties
            [HttpPost("{userId}/favorite-properties")]
            [ProducesResponseType(StatusCodes.Status200OK)]
            [ProducesResponseType(StatusCodes.Status404NotFound)]
            public async Task<ActionResult<ApiResponse<string>>> AddPropertyToUserFavorites([FromServices] UserRepository _userRepository, int userId, [FromBody] int propertyId)
            {
                ...
                user.FavoritePropertyIds?.Add(propertyId);
                await _genericRepository.UpdateAsync(userId,user);

                return Ok(new ApiResponse<string>(true, "Property added to favorites successfully", null));
            }


            // DELETE /api/users/{userId}/favorite-properties/{propertyId}
            [HttpDelete("{userId}/favorite-properties/{propertyId}")]
            [ProducesResponseType(StatusCodes.Status200OK)]
            [ProducesResponseType(StatusCodes.Status404NotFound)]
            public async Task<ActionResult<ApiResponse<string>>> RemovePropertyFromUserFavorites([FromServices] UserRepository _userRepository,int userId, int propertyId)
            {
                var user = await _userRepository.GetByIdAsync(userId);
                ...
                user.FavoritePropertyIds.Remove(propertyId);
                await _userRepository.UpdateAsync(userId, user);

                return Ok(new ApiResponse<string>(true, "Property removed from favorites successfully", null));
            }
        }
    }
